          ...
          <div key={v.num}
            ...
            <div>
              <button className="dots-btn"
                onClick={e => { e.stopPropagation(); showToast('Версия '+v.num+': действия') }}>
                ···
              </button>
            </div>
          </div>
                  ...
                  <tbody>
                    {changeRows.map(r => (
                      <tr key={r.n}>
                        ...
                        <td className="td-o"><span className="clip" title={r.old}>{r.old}</span></td>
                        <td className="td-n"><span className="clip" title={r.nw}>{r.nw}</span></td>
                        ...
                        <td className="td-r"><span className="clip" title={r.rec}>{r.rec}</span></td>
                        <td style={{ textAlign:'center' }}><button className="dots-btn" onClick={e => { e.stopPropagation(); showToast('Строка '+r.n+': действия') }}>···</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export function ComparePage() {
  ...
  const handleVtvClick = (e: React.MouseEvent<HTMLDivElement>) => {
    const el = (e.target as Element).closest('[data-risk]') as HTMLElement | null
    if (!el) { setVtvBar(null); return }
  ...
  return (
    <div className="page" onClick={() => { setVtvBar(null); setShowRiskMenu(false) }}>
      ...
      {tab === 'table' && (
        <div className="tab-pane">
          <div className="tbl-toolbar">
            ...
            {/* Active risk filter badge */}
            {riskFilter !== 'all' ? (
              <span className="filter-badge">
                <span className={`rdot ${riskFilter === 'red' ? 'r' : riskFilter === 'orange' ? 'o' : 'g'}`}/>
                {riskFilter === 'red' ? 'Противоречие' : riskFilter === 'orange' ? 'Проверить' : 'Безопасно'}
                <button className="filter-badge-rm" onClick={() => setRiskFilter('all')}>×</button>
              </span>
            ) : (
              <div style={{ position:'relative' }}>
                <button className="btn-outline" style={{ display:'flex', alignItems:'center', gap:5 }}
                  onClick={() => setShowRiskMenu(prev => !prev)}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  Риск
                </button>
                {showRiskMenu && (
                  <div className="ctx-menu" style={{ top:'100%', left:0, marginTop:4 }}
                    onClick={e => e.stopPropagation()}>
                    {([['red','Противоречие'],['orange','Проверить'],['green','Безопасно']] as const).map(([v,l]) => (
                      <button key={v} className="ctx-item" onClick={() => { setRiskFilter(v); setShowRiskMenu(false) }}>
                        <span className={`rdot ${v === 'red' ? 'r' : v === 'orange' ? 'o' : 'g'}`}/>{l}
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:8 }}>
              ...
              <button className="btn-outline" onClick={() => setDense(!dense)}>
          ...
          <div className="tbl-wrap">
            <table className={`ctbl${dense?' dense':''}`}>
              <colgroup>
              ...
              <thead><tr>
                ...
                <th>Было</th>
                <th>Стало</th>
                <th className="sortable" style={{ width:160 }}
                  onClick={() => { setSortCol(c => { if (c === 'type') { setSortDir(d => d==='asc'?'desc':'asc'); return 'type' } setSortDir('asc'); return 'type' }) }}>
                  Тип изменения&nbsp;
                  <span style={{ color: sortCol==='type' ? 'var(--txt)' : 'var(--faint)', fontSize:10 }}>
                    {sortCol === 'type' ? (sortDir === 'asc' ? '↑' : '↓') : '↑↓'}
                  </span>
                </th>
                <th className="sortable" style={{ width:110 }}
                  onClick={() => { setSortCol(c => { if (c === 'risk') { setSortDir(d => d==='asc'?'desc':'asc'); return 'risk' } setSortDir('asc'); return 'risk' }) }}>
                  Риск&nbsp;
                  <span style={{ color: sortCol==='risk' ? 'var(--txt)' : 'var(--faint)', fontSize:10 }}>
              ...
              <tbody>
                {filtered.map(r => (
                  <tr key={r.n}
                    ...
                    onClick={() => setSelectedRows(s => { const ns=new Set(s); ns.has(r.n)?ns.delete(r.n):ns.add(r.n); return ns })}
                    style={{ cursor:'pointer' }}>
                    <td style={{ color:'var(--faint)', fontSize:12 }}>{r.n}</td>
                    <td className="td-s">{r.s}</td>
                    <td className="td-o"><span className="clip" title={r.old}>{hl(r.old, search)}</span></td>
                    <td className="td-n"><span className="clip" title={r.nw}>{hl(r.nw, search)}</span></td>
                    <td className="td-t">{r.type}</td>
                    <td><span className={`bdg ${rBdg(r.risk)}`}>{rFull(r.risk)}</span></td>
                    <td className="td-m" style={{ fontSize:'11.5px' }}>{r.law}</td>
                    <td className="td-r"><span className="clip" title={r.rec}>{hl(r.rec, search)}</span></td>
                    <td style={{ textAlign:'center' }}><button className="dots-btn" onClick={e=>{e.stopPropagation();showToast('Строка '+r.n)}}>···</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'vtv' && (
        <div className="tab-pane">
          <div className="vtv-wrap" onClick={handleVtvClick}>
          ...
          {vtvBar && (
            <div className="vtv-bar" onClick={e => e.stopPropagation()}>
              ...
              <div className="vtv-bar-right">
                <span className={`bdg ${rBdg(vtvBar.risk)}`}>{rFull(vtvBar.risk)}</span>
