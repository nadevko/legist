...
// - "null" or "public" (case-insensitive): public rows only; admin only
...
func resolveOwnerListQuery(c echo.Context) (ownerListKind, string, error) {
	...
	case strings.EqualFold(ownerRaw, "null") || strings.EqualFold(ownerRaw, "public"):
		if !isAdmin {
			return 0, "", errorf(http.StatusBadRequest, "invalid_parameter_value",
				"owner=null or owner=public is only allowed for admins", "owner")
		}
		return ownerListPublicOnly, "", nil
	default:
		if ownerRaw != uid {
			return 0, "", errorf(http.StatusBadRequest, "invalid_parameter_value",
				"owner must be your user id, null, or public (null/public for admins only)", "owner")
		}
		return ownerListSelfOnly, uid, nil
	}
}
